...
(function(){
    // 1. 获取DOM元素
    var wrapper = document.querySelector('.wrapper-small');
    var prevBtn = document.querySelector('.prev-btn-small');
    var nextBtn = document.querySelector('.next-btn-small');
    var idots = Array.from(document.querySelectorAll('.pagenation-item-small'));
    var curIndex = 1;
    // 记录当前是否在做切换
    ...
    idots.forEach(function(idot,index){
        ...
        idot.onclick = function() {
            ...
            // 计算偏移
            var offset = -(index - curIndex) *  1000;
            // 切换
            tab(offset);
            // 更新下标显示
            curIndex = index ;
